...
import { shouldUseCloudApi } from './apiClient';
...
export const compileFile = async (srcPath: string): Promise<boolean> => {
    ...
    // If cloud API is enabled and language is supported, skip local compilation
    // The API will handle both compilation and execution together
    if (shouldUseCloudApi(language)) {
        globalThis.logger.log('Using cloud API - skipping local compilation');
